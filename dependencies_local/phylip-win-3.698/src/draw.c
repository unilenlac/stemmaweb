...
778,1000,365,0,0,0,0,0,889,0,0,0,278,0,0,222,611,944,611,0,0,0,0}; /* height */
static short helveticabold_metric[] = {718,
...
778,1000,365,0,0,0,0,0,889,0,0,0,278,0,0,278,611,944,611,0,0,0,0}; /* height */
...
722,889,310,0,0,0,0,0,667,0,0,0,278,0,0,278,500,722,500,0,0,0,0};
static short timesitalic_metric[] = {660, 
...
722,944,310,0,0,0,0,0,667,0,0,0,278,0,0,278,500,667,500,0,0,0,0}; /* height */
static short timesbold_metric[] = {681, 
...
1000,330,0,0,0,0,0,722,0,0,0,278,0,0,278,500,722,556,0,0,0,0}; /* height */
static short timesbolditalic_metric[] = {662, 
...
,722,944,300,0,0,0,0,0,722,0,0,0,278,0,0,278,500,722,500,0,0,0,0}; /* height */
 ...
 *
 *
 * %%DocumentMedia: Page <width> <height> <weight> <color> <type>
 * leave out weight, color, type, leaves:
 * %%DocumentMedia Page <width> <height> 0 ( ) ( )
 *
 *********************************************************************/
...
/*
  ...
  */
  i=0;
  openfile(&fontfile,fontname,"font file","r",application,NULL);
...
)
{
  ...
  if (dotmatrix){
    ...
    for (j=0; j<ypag; ++j)
      for (i=0; i<xpag; ++i){
        ...
        //printf("paperx: %f  papery: %f  clipx0: %f clipx1: %f  clipy0: %f clipy1: %f\n", paperx, papery, clipx0, //clipx1,clipy0, clipy1); /* debug */
        plottree(root,root);
        plotlabels(fontname);
  ...
  if (dotmatrix){
    ...
    //printf("in drawit dot matrix code\n");
   //printf("DEFAULT_STRIPE_HEIGHT: %i\n", DEFAULT_STRIPE_HEIGHT);
    //printf("calling striprint ysize: %f yunitspercm: %f numlines: %li strpdeep: %li\n",  ysize, yunitspercm, numlines , strpdeep);
    //fflush(stdout);
...
//           fflush(stdout);
      ...
      xnow = strpwide / 2.0;
      ynow = 0.0;
      //printf(" calling plottree root: %p\n", root);
      //fflush(stdout);
      plottree(root, root);
      plotlabels(fontname);
      ...
      //printf("strptop: %li strpbottom: %li\n",strptop,strpbottom);
      //printf(" strpdeep: %li DEFAULT_STRIPE_HEIGHT: %i\n", strpdeep, DEFAULT_STRIPE_HEIGHT);
      //fflush(stdout);
      if (strpdeep > DEFAULT_STRIPE_HEIGHT){
        ...
        //printf(" calling striprint\n");
        //fflush(stdout);
        //printf(" small stripe strpdiv: %li strpdeep: %li\n",strpdiv,strpdeep);
        //fflush(stdout);
